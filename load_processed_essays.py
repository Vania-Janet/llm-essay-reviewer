...
def find_matching_anexo(author: str, anexos_dict: dict) -> tuple:
    """
    Busca el anexo correspondiente al autor del ensayo.
    Retorna (ruta_anexo, texto_anexo) o (None, None) si no se encuentra.
    """
    # Normalizar nombre del autor para búsqueda
    author_normalized = author.lower().replace(' ', '_')
    
    for anexo_path, anexo_text in anexos_dict.items():
        anexo_filename = Path(anexo_path).stem.lower()
        
        # Buscar coincidencia en el nombre del archivo
        if author_normalized in anexo_filename:
            return (str(anexo_path), anexo_text)
        
        # También buscar partes del nombre
        author_parts = author.split()
        if len(author_parts) >= 2:
            # Buscar por apellido y primer nombre
            if all(part.lower() in anexo_filename for part in author_parts[:2]):
                return (str(anexo_path), anexo_text)
...
def process_all_essays():
    ...
    # Carpetas de trabajo
    pdfs_folder = Path(__file__).parent / 'pdfs_procesado'
    anexos_folder = Path(__file__).parent / 'Anexo_procesado'
    
    if not pdfs_folder.exists():
        print(f"❌ Carpeta no encontrada: {pdfs_folder}")
        return
    
    # Cargar todos los anexos en memoria
    anexos_dict = load_all_anexos(anexos_folder)
    ...
    with app.app_context():
        for txt_file in tqdm(txt_files, desc="Procesando ensayos"):
            try:
                ...
                # Buscar anexo correspondiente
                ruta_anexo, texto_anexo = find_matching_anexo(author, anexos_dict)
