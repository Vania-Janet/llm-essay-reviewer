...
function renderEssayQueue() {
    ...
    queueList.innerHTML = currentEssays.map(essay => {
        ...
        // Check if essay has been evaluated (this would come from evaluaciones_jurado table)
        const isDone = false; // TODO: Check from backend
        ...
        return `
            <div class="queue-item ${statusClass}" data-essay-id="${essay.id}">
                ...
                <div class="meta">
                    ...
                    ${isDone ? `<span class="essay-score">4.5/5.0</span>` : ''}
                </div>
            </div>
        `;
    }).join('');
...
function loadPdfInViewer(essayId) {
    ...
    // Load PDF
    pdfFrame.src = `/essays/${essayId}/pdf`;
